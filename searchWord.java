...
public class searchWord {
	...
	public static class TokenizerMapper extends Mapper<Object, Text, Text, IntWritable>{
		...
		public void map(Object key, Text value, Context context) throws  IOException, InterruptedException {	
			...
			String[] indexString = value.toString().split("\t");// get the work and passage
			
			String searchString = "hello world";
			String[] searchSplit = searchString.split(" ");
			
			for(String val:searchSplit){
				if(val == indexString[0]){
					String[] passageSplit = indexString[1].split("$");
					int i = passageSplit.length-1;
					while(i>=0){
						context.write(new Text(passageSplit[i]), one);
						i--;
					}
				}
			}
